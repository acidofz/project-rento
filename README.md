...
- Мониторинг: логи деплоя, алерты при падении сервиса, периодическая проверка `/health` и ключевых сценариев (вход, лента, чат).
- На Railway healthcheck платформы настроен на **`/ping`** (лёгкий endpoint Reflex); страница **`/health`** — для ручной проверки в браузере.
